...
export async function main(ns) {
    //set cloud targetCloud name from args
    ...
    ns.print("1");
    // main behaviour if no target specified in args
    if (!targetCloud) {
        for (const cloudName in clouds) {
            ns.tprint("iterating: " + cloudName);
            await update(ns, cloudName);
        }
    }
    else {
        ns.print("2");
        await update(ns, targetCloud);
    }
}
